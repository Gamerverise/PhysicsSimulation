...
public class Struc<N extends Enum<N>, V> {
    V[] values;

    public Struc(Class<N> clazz) {
        int num_fields = clazz.getClass().getEnumConstants().length;
        values = (V[]) new Object[num_fields];
    }

    public Struc(StructField<N, V> field, StructField<N, V>... fields) {
    ...
    static <N extends Enum<N>, V> Struc<N, V> S(Class<N> clazz) {
        return new Struct(clazz);
    }

    static <N extends Enum<N>, V> Struc<N, V> S(StructField<N, V> field, StructField<N, V>... fields) {
